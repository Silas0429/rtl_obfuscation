...
| T028 | `READY` | 对闭包执行五类对象的选择、加密、mapping、重编译、解密和小型工程 formal | 是 |
...
本路线图不是活动任务合同。T027 已验收，T028 合同已冻结并处于 `READY`；T028 未
`ACCEPTED` 前不得建立 T029。T006 继续保持 `DRAFT`，本路线图不实现或重命名 parameter。
...
形式验收必须使用提交到仓库的 integration fixture：

```sh
conda run -n rtl_obfuscation python scripts/formal_equivalence.py \
  --gold-filelist <generated-gold-closure-filelist> \
  --gold-root tests/fixtures/project_root/integration \
  --gate-filelist <generated-gate-closure-filelist> \
  --gate-root <gate-dir> \
  --top project_top
```
...
五个独立 category 组、组合组、四个漏改负例、formal 正负例、解密 hash、legacy 回归、完整
unittest 和 `git diff --check` 必须在一次交付中全部通过。不得以“某类后续任务再处理”为理由
提交部分 category。
...
2. `inspect-project`：17 个 reachable module、19 个 closure files、0 parse error、
   0 semantic error。
3. 使用五个概念组组合加密；mapping 中 parameter entries 必须为 0。
...
9. gold/gate formal view 的移除清单除名称改写造成的 hash 差异外，语法类别、文件和结构位置
   一一对应。
10. Yosys formal 正例通过；人为修改一个 reachable 算术/控制逻辑后的负例失败。
11. T027 integration fixture 和旧 filelist FIFO 的所有回归继续通过。
