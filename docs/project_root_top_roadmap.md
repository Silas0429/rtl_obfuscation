...
| T038 | `BLOCKED` | RISC-V-Vector parameter/genvar 修复与加密率分母统一；旧合同不再继续扩展 | 是 |
...
当前任务是 T038，状态为 `BLOCKED / NOT_ACCEPTED`。已验证同名 generate-loop genvar 不再被参数
收集器误认，且非 RISC 显式回归为 `Ran 115 tests`、`OK`；但六组 RISC profile 仍实际产生
`1211 / 6882`（entries/occurrences），未达到合同冻结的 `1211 / 6835`，专用 fixture、验收脚本
和 RISC Formal 正负例也尚未完成。
...
主 Agent 独立重跑目标 unittest、CLI 正负例、oracle 比较和排除 `tests.test_risc_v_vector_project_root`
的常规回归；对 T028/T029 还要独立重跑 gate compilation、解密 hash、formal 正例和非等价负例。
RISC-V-Vector Formal 只属于专门 RISC 验收任务。验收只依据退出码、JSON、
