...
| T034 | `READY` | 单文件/filelist 默认 single-module profile 与 multi/ABI fail-closed | 是 |
...
本路线图不是活动任务合同。T027—T033 已验收，T034 已冻结为 `READY`，T006 继续保持 `DRAFT`；parameter inventory
...
T033 已冻结 impact/category oracle；T034–T037 的 profile 迁移和 RISC 参数集成尚未实现。
