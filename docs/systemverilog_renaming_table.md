...
| `ports` | 非 top module 普通 port | 声明、module body、named connection 左侧 | `child u(.data(x));` | top 普通 ports 始终保留；不处理外部约束 | 17/59 |
...
- 加密输入可以是显式 filelist，也可以由 `project-root + top` 自动建立已验收子集的 active
  include/宏闭包。project-root 模式只开放 `signals/ports/instances/struct/interface` 五个概念组，
  parameter、module、union_fields 和其他 legacy category 不在该模式范围内。
- 只修改输入 RTL，不同步修改 testbench、SDC、Tcl、软件模型和外部层次路径。
- 不支持 type parameter、package/class parameter、DPI、bind、class、clocking block、
  virtual interface 和任意完整 SystemVerilog lexical shadow。
