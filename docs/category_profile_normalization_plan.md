...
- `multi_module` 或 ABI 对象只能由 `project-root + top` 手动选择；
- filelist 编译和输出所有列出的文件，但只修改其中符合默认 profile 的 ranges；
...
### 3.2 project-root 手动类别

下列类别默认不启用，只有 project-root 手动选择时才允许改写：
...
- filelist：与单文件使用同一默认 profile；multi/ABI category 返回稳定错误；不以 top
  建立闭包；
...
旧 project-root 的 `struct`、`interface` 概念组保留为兼容 alias，但 registry 的 canonical
category 仍是实际底层 category。旧 v1/v2/v3 mapping 继续支持 decrypt。

## 5. Mapping/metrics 方案

新的 normalized workflow 建议使用 mapping v4，记录：
...
`skipped` 必须记录 filelist 默认 profile 跳过的 multi-module、ABI、macro 和 unsupported
对象及稳定 reason。旧 mapping 只读兼容，不回写旧 schema。
...
| T035 | project-root 手动 multi-module profile、跨 module parameter/ABI | 是 | 必须 PASS/FAIL 正负例 |
| T036 | RISC-V-Vector `v_int_alu` 参数集成与 formal-view | 是 | 必须 PASS/FAIL 正负例 |
| T037 | 条件性默认 profile 晋级与 oracle 重冻结 | 是 | 复用 T036 |
...
2. filelist 不改写 multi-module/ABI ranges，并输出可审计 skip；
3. project-root 默认只改写 top closure；
4. project-root 手动 profile 可改写 closure 内 multi-module ranges，但 top ABI 保持；
