...
预期：17 tests 全部通过；encrypt/decrypt 均报告 `23/63`；三个前端检查退出码
均为 0；formal JSON 为 `formal_equivalence=pass`；`cmp` 退出码为 0。

## 8. 推荐下一任务

T013 已验收 `typedefs` 和 `struct_types`。下一步 T014 规划 `struct_fields` 和 `union_fields`，继续复用现有 mapping、随机名称、
