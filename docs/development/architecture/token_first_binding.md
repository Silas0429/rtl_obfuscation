...
## 6. 产品目标与后续路线（2026-08-27 用户确认）

本项目服务于对外售卖 IP 的混淆需求，**不要求完全加密**：`--encryption-rate` 本就支持部分加密。
最低要求是 `signals`、`ports`、`interface`、`struct` 四组**稳定改名且不报错**。

这个定义改变了优先级：目标不是提升覆盖率，而是让每组都有**可证明正确的真实改名**，
无法证明的对象逐个保留并给出可解释原因。

### 6.1 两类残差必须区分，否则会做错优先级

| 口径 | StCache 上的残差 | 含义 |
| --- | --- | --- |
| **产品** `rename_index.py` | **3 个** `source_binding_incomplete` signature | 产品只尝试绑定它有规则的东西，只在"有语义目标但映射不到 token"时报告 |
| **探测器** `binding_coverage.py` | 11182 token，47% 未定性 | 探测器做全 token 完整性核算，任何未归属都计入 |

产品要达到最低要求，需要修的是那 3 个形状加上爆炸半径，**不需要**先解决探测器口径下的 47%。
探测器口径只有在采用逐符号完整性判据（§2）作为保证机制时才成为门槛。
