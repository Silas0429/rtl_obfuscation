...
| `rtl_obfuscator/project_discovery.py` | 运行 PySlang 编译/elaboration；parse 后在 SourceCatalog 全树遍历或 FAST 改名索引开始前核对真实 source/include buffer；按诊断码和物理字节精确分类已验证供应商诊断 |
...
`DesignFile`、`LibraryFile`、`IncludeFile` buffer 是否已登记，未登记时在 SourceCatalog 全树遍历或 FAST 改名索引开始前带路径拒绝。
