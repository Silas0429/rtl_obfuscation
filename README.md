...
168 个被改写 token，PySlang/Verible 前端检查和字节级解密恢复均通过。样例还展示了
内部 interface signal bundle 连接到 `fifo_ctrl` 的 `fifo_if.consumer ctrl` 端口，以及 packed struct
作为 function argument 的实际使用；`fifo_top` 对外 ABI 仍保持普通标量/向量 ports。
...
conda run -n rtl_obfuscation python -m unittest \
  tests.test_all_category_rewrite tests.test_debug_mode tests.test_encrypt_demo \
  ...
  tests.test_t035_profile_unification tests.test_t036_encryption_rate \
...
当前显式非 RISC 回归为 `Ran 115 tests`、`OK`（3 项当前 FIFO Yosys formal 按接口端口边界跳过）。
该清单明确排除 `tests.test_risc_v_vector_project_root`；RISC-V-Vector Formal 只由活动的专门验收任务启动。
...
三种加密入口都支持可选的 `--encryption-rate <rate>`，例如：
...
conda run -n rtl_obfuscation python -m rtl_obfuscator.rewrite encrypt-project \
  ...
  --top fifo_top \
  --output-dir /tmp/rtl_fifo/rate-gate \
  --map /tmp/rtl_fifo/rate-mapping.json \
  --metrics /tmp/rtl_fifo/rate-metrics.json \
...
### 3.5 多文件样例 `rtl_samples/example_fifo`
...
├── fifo_ctrl.sv     # FIFO 状态、读写指针、enum、generate 和 fifo_if.consumer ctrl 端口
...
`fifo_top` 对外是普通 ports，`fifo_if` 在 top 内部实例化，并通过 `.ctrl(fifo_bus)` 连接到
`fifo_ctrl` 的 `fifo_if.consumer ctrl` 端口。因此本样例可以显式开启 interface 相关 category；
`fifo_bus` 的成员实际承载 top 与 controller 之间的 FIFO 控制、数据和状态连接，但它不代表已支持
“顶层 interface port”。`fifo_storage` 中的 `extract_payload(view.entry)`
则展示了 `fifo_entry_t` packed struct 作为 function argument 传递。

### 3.6 多文件完整流程

下面按“加密→前端检查→解密”执行。由于该样例刻意包含下层 module 的 interface-typed port，
当前 Icarus/Yosys formal 适配层不支持这一语法形状；演示路径使用 PySlang 和 Verible 做轻量检查，
Formal 正例/负例保留在不含该边界的专用 fixture 中。
...
1. 加密：
   ...
   预期摘要为 `{"files": 4, "mapping_entries": 67, "modified_tokens": 268}`。输出目录与 gold 分开：
...
2. 前端检查：
   ...
   ```sh
   conda run -n rtl_obfuscation verible-verilog-syntax /tmp/rtl_fifo/gate/fifo_ctrl.sv
   ```

   同时用 PySlang 严格解析 gate filelist；两项都应退出码为 0。
...
如果只是演示当前样例的加密和解密，可以直接运行：

```sh
conda run -n rtl_obfuscation python encrypt.py
```

脚本支持两个样例，并对每个样例启用 registry 中的全部 19 个 canonical category，默认名称长度
为 20，不执行耗时的 Formal：

```sh
# RISC-V-Vector，默认输出 /tmp/rtl_samples/riscv
conda run -n rtl_obfuscation python encrypt.py --sample riscv

# FIFO，默认输出 /tmp/rtl_samples/fifo
conda run -n rtl_obfuscation python encrypt.py --sample fifo
```

每次运行依次生成 `gate/`、`mapping.json`、`metrics.json`、per-file `maps/` 和 `restored/`，
最后输出 JSON 摘要并确认所有 top closure 文件 byte-identical。可用 `--work-dir <dir>` 指定一个
不存在或空的输出目录，`--name-length <int>` 覆盖默认 20，或用 `--encryption-rate <rate>`
演示 T036 的选择功能。当前轻量结果为：FIFO 4 files / 67 entries / 268 tokens；RISC-V-Vector
19 files / 1238 entries / 7081 tokens。
...
加密命令成功只表示完成了改写，不表示 gate 已经可交付。对 Yosys 支持的 RTL，应执行
PySlang 和 Yosys；T035 常规验收只覆盖非 RISC fixture，RISC-V-Vector 的
`formal-view`/`formal-align`/Yosys 仅在专门 RISC 验收任务中执行。当前 `example_fifo`
因刻意使用下层 interface-typed port，演示验收改用 PySlang、Verible、gate audit 和 decrypt。
...
Yosys 形式等价（仅适用于 Yosys 支持的 fixture）：
...
parameter 可能被误收为 module parameter；该边界由仍处于 `IN_PROGRESS` 的 T038 专项修复。
T038 当前六组 RISC profile 可以完成 gate/decrypt，但实际为 `1211 entries / 6882 occurrences`，
与合同冻结的 `6835` occurrences 仍有偏差，且专用紧凑 fixture、验收脚本和 RISC Formal 正负例
尚未完成；在 T038 `ACCEPTED` 前，不要把该六组 profile 作为已验收能力。根目录 `encrypt.py`
的全类别 FIFO/RISC 演示是单独的用户演示入口，不替代 T038 Formal 验收。
...
特别注意：`example_fifo` 的 interface 是 `fifo_top` 内部实例，但现在作为
`fifo_ctrl` 的 `fifo_if.consumer ctrl` 端口传入；top 对外仍是普通 ports，因此不等同于
“顶层 module 使用 interface port”。后者仍列入[未来事项](docs/future_work.md)。
