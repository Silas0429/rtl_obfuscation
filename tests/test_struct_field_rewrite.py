"""Black-box tests for struct_fields category rewriting."""
...
class StructFieldRewriteCliTest(unittest.TestCase):
    def test_struct_field_encrypt_decrypt_round_trip(self) -> None:
        repository = Path(__file__).resolve().parents[1]
        gold = Path("tests/fixtures/t014_struct_field.sv")
        ...
        with TemporaryDirectory() as tmp:
            ...
            expected = [
                ("low_nibble", 142, 152, [(301, 311), (475, 485)]),
                ("high_nibble", 174, 185, [(349, 360), (448, 459)]),
            ]

            for entry, (name, decl_start, decl_end, refs) in zip(entries, expected):
                self.assertEqual(entry["category"], "struct_fields")
                self.assertEqual(entry["scope"], "t014_struct_field")
                ...
                self.assertEqual(entry["declaration"], {
                    "file": "tests/fixtures/t014_struct_field.sv",
                    "start": decl_start,
                    "end": decl_end,
                })
                self.assertEqual(entry["references"], [
                    {"file": "tests/fixtures/t014_struct_field.sv", "start": s, "end": e}
                    for s, e in refs
                ])
