"""Black-box FAST contract for compile-oriented flattened filelist delivery."""
    ...
    def _encrypt(cls, gate: Path) -> Path:
        result = subprocess.run(
            (
                ...
                "--top", "t149_top", "--rewrite-root", str(cls.project / "rtl"),
    ...
    def test_local_include_change_is_logged_without_rewriting_copy(self):
        ...
        result = subprocess.run(
            (
                ...
                "--top", "t149_unsafe", "--rewrite-root", str(project / "rtl"),
                "--category", "signals", "--output-dir", str(gate),
            ),
            cwd=ROOT, capture_output=True, text=True, timeout=180, check=False,
        )
