"""Black-box tests for typedefs category rewriting."""
...
class TypedefRewriteCliTest(unittest.TestCase):
    def test_typedef_encrypt_decrypt_round_trip(self) -> None:
        repository = Path(__file__).resolve().parents[1]
        gold = Path("tests/fixtures/t013_typedef.sv")
        ...
        with TemporaryDirectory() as tmp:
            ...
            entry = entries[0]
            self.assertEqual(entry["category"], "typedefs")
            self.assertEqual(entry["scope"], "t013_typedef")
            self.assertEqual(entry["original_name"], "byte_t")
            self.assertEqual(entry["declaration"], {
                "file": "tests/fixtures/t013_typedef.sv",
                "start": 113,
                "end": 119,
            })
            ...
            self.assertNotIn(b"byte_t", gate_bytes)
